...
def diffusion_action_generation(count, image, depth, status, Base2Cam_RT, policy, seq_data, seq_length= 5):
    ...
    #print('diffusion output EE_vector: ', EE_vector)
    ...
    if np.any(np.array(pred_action[7:]) <0.035):
        EE_vector = np.array([-0.01,-0.01])
    else:
        EE_vector = np.array([0.04,0.04])

    jump = 5
    if np.any(np.array(status['EE']) > 0.035) and np.any(EE_vector < 0.02):
...
fruits_size_list = {'food_apple':[0.8,0.8,0.8], 'food_plum':[1,1,1], 'food_lemon':[1.05,1.05,1.05]}
fruits_orientation = {'food_apple': [0.0, 0.0, 0.0], 'food_plum': [0.0, 0.0, 0.0],  'food_lemon': [0.0, 0.0, 0.0]}

fruits_location = {'food_apple': [0.8, 0.2, 0.3], 'food_plum': [0.8, 0.0, 0.3], 'food_lemon': [0.8, -0.2, 0.3]}
fruits_pick_high = {'food_apple':[0, 0, 0.035], 'food_plum':[0, 0, 0.035], 'food_lemon':[0.0, 0, 0.035]}

usd_home = '/home/jtl/ISAAC/cross_perspective_robotics_manipulation/calibration-diffusion/objects/usd/fruits/'
fruits_names = list(fruits_size_list.keys())
...
model_name = '/home/jtl/ISAAC/cross_perspective_robotics_manipulation/Calibration_Diffusion/0410_Franka_Classify_Fruits_diffusion_sample_300_1200.pth'
print(model_name)
...
if __name__=="__main__":  
    ...
    controller = PickPlaceController(
        ...
        events_dt = [0.008, 0.005, 1, 0.02, 0.05, 0.01, 0.008, 0.05, 0.008, 0.08]
    )
    '''
    ...
    my_world.scene.add(
        FixedCuboid(
            ...
            name="fixed_cube",
            position=np.array([0.5 , 0.0, 0.18]),
            scale=np.array([0.4, 0.6, 0.02]),
    ...
    my_world.scene.add(
        FixedCuboid(
            ...
            name="fixed_cube_fruits_forward",
            position=np.array([0.9 , 0.0, 0.1]),
            scale=np.array([0.02, 0.6, 0.2]),
            color=np.array([0.1, 0.1, 0.1])
        )
    )
    ...
    my_world.scene.add(
        FixedCuboid(
            ...
            name="fixed_cube_fruits_clip1",
            position=np.array([0.8 , -0.3, 0.1]),
    ...
    # my_world.scene.add(
    #     FixedCuboid(
    #         prim_path="/World/fixed_cube14",
    #         name="fixed_cube_fruits_clip7",
    #         position=np.array([0.8 , -0.2, 0.1]),
    #         scale=np.array([0.2, 0.01, 0.2]),
    #         color=np.array([0.1, 0.1, 0.1])
    #     )
    # )
    ...
    #         name="fixed_cube_fruits_clip3",
    #         position=np.array([0.8 , 0.0, 0.1]),
    #         scale=np.array([0.2, 0.01, 0.2]),
    #         color=np.array([0.1, 0.1, 0.1])
    #     )
    # )
    ...
    for fruit_num in range(3):        
        size = fruits_size_list[fruits_names[fruit_num]]
        loc = fruits_location[fruits_names[fruit_num]]
        food_name = "ready_{}_{}".format(fruits_names[fruit_num],fruit_num)
        prim_path = "/World/"+food_name
        add_reference_to_stage(usd_path=usd_home + fruits_names[fruit_num]+".usd", prim_path=prim_path)
        food = Articulation(prim_path=prim_path,
                    name= food_name,
                    position=np.array(loc),
                    scale = np.array(size)
                    ) 
        my_world.scene.add(food)
    my_world.reset()
    ...
    while simulation_app.is_running():
        ...
        for t in range(50):
            ...
            for fruit_dict in fruits:
                for k in list(fruit_dict.keys()):
                    #my_world.scene.remove_object(name="random_red_cube_{:d}".format(cube_index))
                    my_world.scene.remove_object(name=k)


            my_world.reset()

            random_element = random.choice(fruits_names)
            random_x = random.uniform(0.5, 0.7)
            random_y = random.uniform(-0.25, 0.25)

            size = fruits_size_list[random_element]
            orien = fruits_orientation[random_element]
            food_name = "{}".format(random_element)
            prim_path = "/World/"+food_name
            add_reference_to_stage(usd_path=usd_home + random_element+".usd", prim_path=prim_path)
            food = Articulation(prim_path=prim_path,
                        name= food_name,
                        position=np.array([random_x,  random_y,  0.185]),
                        orientation = euler_angles_to_quat(np.array([orien[0],orien[1],orien[1]])),
                        scale = np.array(size))
            my_world.scene.add(food)
            fruits.append({food_name:{'food':food}})

            # camera_x = random.uniform(1.6, 2.2)
            # camera_y = random.uniform(-1, 1)
            # #camera_y = 0 
            # #camera_z = random.uniform(1.0, 1.3)
            # camera_z = random.uniform(1.5, 1.8)
            ...
            camera_delta_pitch = math.atan2(camera_z-0.2,math.sqrt((camera_x-0.6)**2 + (camera_y)**2))
            ...
            # RT_path = os.path.join("test", "RT.npy")
            # np.save(RT_path, RT) 
            ...
            while i < 500:
                ...
                #         target_position=np.array([0.52, 0, 0.25]),
                #         target_orientation= np.array([0.15, 0, 0.991057521791512, 0]))
                ...
                if jump == 0:
                    target_position, target_orientation, gripper_close, seq_data , jump =  diffusion_action_generation(i-20, bgr_img, depth, franka_arm_para, RT, policy, seq_data, seq_length= cfg.set_seq_length)
                    ...
                    # image_path = os.path.join("test", f"{img_count + 1}.jpg")
                    # depth_path = os.path.join("test", f"{img_count + 1}.npy")
                    # json_path = os.path.join("test", f"{img_count + 1}.json")
                    
                    # np.save(depth_path, depth)
                    # cv2.imwrite(image_path, bgr_img)

                    # # json_data = json.dumps(franka_arm_para)
                ...
                food_pose = food.get_world_pose()[0] 
                distance = np.sqrt(np.sum((np.array(food_pose)[:2] - np.array(fruits_location[food_name])[:2]) ** 2))
                #print(distance)
                if distance < 0.1 and np.array(food_pose)[2] < 0.2:
                    success_count += 1 
                    print('success times {:d}, current eposide {:d}'.format(success_count, t+1))
                    break
            ...
            gif_image[0].save('diffusion_classify_fruits_{:d}.gif'.format(t),
               save_all=True,
