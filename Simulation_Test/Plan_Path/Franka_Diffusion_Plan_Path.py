...
model_name = "0410_Franka_Plan_Path_diffusion_sample_300_1600.pth"
print(model_name)
...
if __name__=="__main__":  
    ...
    while simulation_app.is_running():
        ...
        for t in range(50):
            ...
            camera_x = random.uniform(1, 2.5)
            camera_y = random.uniform(-2, 0.5)
            camera_z = random.uniform(1.8, 3)
 
            camera_delta_yaw = math.atan2(camera_y,(camera_x-0.4))
            camera_delta_pitch = math.atan2(camera_z,math.sqrt((camera_x-0.4)**2 + (camera_y+0.3)**2))
            ...
            #机械臂回初始位置
            franka.set_joint_positions(np.array([0.23169483,  0.5348021,   0.26620936, -1.6795475,  -0.16399851,  2.1930306, 1.3452059, 0.04, 0.04]))

            random_p1 = random.uniform(0.3, 0.75)
            center_1 = [(0.2+random_p1) / 2 -0.03, 0.2, 0.1]
            center_2 = [(0.9+random_p1) / 2 +0.03, 0.2, 0.1]
                
            my_world.scene.add(
                    DynamicCuboid(
                        prim_path="/World/random_red_cube_1",
                        name="random_red_cube_1",
                        position=np.array(center_1),
                        orientation = rot_utils.euler_angles_to_quats(np.array([0, 0, 0]), degrees=False),
                        scale=np.array([(center_1[0] - 0.2)*2, 0.02, 0.1]),
                        color=np.array([0.6, 0, 0.1]),
                    ))

                
            my_world.scene.add(
                    DynamicCuboid(
                        prim_path="/World/random_red_cube_2",
                        name="random_red_cube_2",
                        position=np.array(center_2),
                        orientation = rot_utils.euler_angles_to_quats(np.array([0, 0, 0]), degrees=False),
                        scale=np.array([(0.9 - center_2[0])*2, 0.01, 0.1]),
                        color=np.array([0.0, 0.5, 0.1]),
                    ))

            random_p2 = random.uniform(0.3, 0.75)
            center_3 = [(0.2+random_p2) / 2 -0.03, 0.0, 0.1]
            center_4 = [(0.9+random_p2) / 2 +0.03, 0.0, 0.1]
                
            my_world.scene.add(
                    DynamicCuboid(
                        prim_path="/World/random_red_cube_3",
                        name="random_red_cube_3",
                        position=np.array(center_3),
                        orientation = rot_utils.euler_angles_to_quats(np.array([0, 0, 0]), degrees=False),
                        scale=np.array([(center_3[0] - 0.2)*2, 0.02, 0.1]),
                        color=np.array([0.6, 0, 0.1]),
                    ))
            ...
            my_world.scene.add(
                    DynamicCuboid(
                        prim_path="/World/random_red_cube_4",
                        name="random_red_cube_4",
                        position=np.array(center_4),
                        orientation = rot_utils.euler_angles_to_quats(np.array([0, 0, 0]), degrees=False),
                        scale=np.array([(0.9 - center_4[0])*2, 0.02, 0.1]),
                        color=np.array([0.0, 0.5, 0.1]),
                    ))


            random_p3 = random.uniform(0.3, 0.75)
            center_5 = [(0.2+random_p3) / 2 -0.03, -0.2, 0.1]
            center_6 = [(0.9+random_p3) / 2 +0.03, -0.2, 0.1]
                
            my_world.scene.add(
                    DynamicCuboid(
                        prim_path="/World/random_red_cube_5",
                        name="random_red_cube_5",
                        position=np.array(center_5),
                        orientation = rot_utils.euler_angles_to_quats(np.array([0, 0, 0]), degrees=False),
                        scale=np.array([(center_5[0] - 0.2)*2, 0.02, 0.1]),
                        color=np.array([0.6, 0, 0.1]),
                    ))
            ...
            my_world.scene.add(
                    DynamicCuboid(
                        prim_path="/World/random_red_cube_6",
                        name="random_red_cube_6",
                        position=np.array(center_6),
                        orientation = rot_utils.euler_angles_to_quats(np.array([0, 0, 0]), degrees=False),
                        scale=np.array([(0.9 - center_6[0])*2, 0.02, 0.1]),
                        color=np.array([0.0, 0.5, 0.1]),
                    ))

            my_world.scene.add(
                    DynamicCuboid(
                        prim_path="/World/random_red_cube_7",
                        name="random_red_cube_7",
                        position=np.array([0.5, -0.3, 0.1]),
                        orientation = rot_utils.euler_angles_to_quats(np.array([0, 0, 0]), degrees=False),
                        scale=np.array([0.06, 0.06, 0.01]),
                        color=np.array([0.0, 0.1, 0.6]),
                    ))
            ...
            my_world.reset()
            Franka_init_x = random.uniform(0.3, 0.8)
            ...
            while i < 2500:
                ...
                if i < 20:
                    ...
                    actions, succ = my_IK.compute_inverse_kinematics(
                        target_position=np.array([Franka_init_x, 0.3, 0.20]),
                        target_orientation= euler_angles_to_quat(np.array([3.14, 0, 3.14])))
                    franka.apply_action(actions)
                    #print(matrix_to_euler_angles(my_IK.compute_end_effector_pose()[1]))
                    continue
                ...
                current_joint_positions = franka.get_joint_positions()
                if np.max(np.abs(current_joint_positions))>3:
                    stage = 7

                for cube_index, cube in enumerate(cube_names):
                    #print(init_cube_pos[cube_index], cube.get_world_pose()[0])
                    cube_pos_delta = np.max(np.abs(init_cube_pos[cube_index] - cube.get_world_pose()[0]))
                    if cube_pos_delta > 0.01:
                        stage = 7
                        print(cube_pos_delta, 'failed')
                        break
                
                current_pos = my_IK.compute_end_effector_pose()[0].tolist()
                pos_delta = np.max(np.abs(np.array([0.5, -0.3, 0.2]) - np.array(current_pos)))
                #print(pos_delta)
                if pos_delta<0.01:
                    success_count += 1 
                    print('success times {:d}, current eposide {:d}'.format(success_count, t+1))
            ...
            gif_image[0].save('diffusion_plan_path_{:d}.gif'.format(t),
               save_all=True,
